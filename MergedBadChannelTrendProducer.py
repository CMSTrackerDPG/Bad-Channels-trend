...
if os.path.isfile("list_missingTrackerMap_ZeroBias.txt") and dataset == "ZeroBias": os.remove("list_missingTrackerMap_ZeroBias.txt")
...
#  file_in=open("sistrip_qtest_"+ str(k).strip('\n') +".log","r");
	...
	elif ( dataset == "ZeroBias" and os.path.isfile("/data/users/event_display/Data2016/" + runtype + "/" + runshort + "/"+str(k).strip(' \n')+"/ZeroBias1/MergedBadComponents_run"+str(k).strip(' \n')+".txt")):
		tot=0
		tib=0 
		tob=0 
		tec=0 
		...
		runlist_missingTrackerMap = open('list_missingTrackerMap_' + dataset + '.txt','a+')
...
trend.Draw()
Rleg.Draw()
c_SiStrip.SaveAs(folder+"/MergedBadChannelsTrends/MergedBadChannelsSiStrip_"+str(runmin).strip(' \n')+"_"+str(runmax).strip(' \n')+".png")
c_SiStrip.Write()
...
SetHRange(trendTIB)
trendTIB.Draw()
c_TIB.SaveAs(folder+"/MergedBadChannelsTrends/MergedBadChannelsTIB_"+str(runmin).strip(' \n')+"_"+str(runmax).strip(' \n')+".png")
c_TIB.Write()
...
SetHRange(trendTOB)
trendTOB.Draw()
c_TOB.SaveAs(folder+"/MergedBadChannelsTrends/MergedBadChannelsTOB_"+str(runmin).strip(' \n')+"_"+str(runmax).strip(' \n')+".png")
c_TOB.Write()
...
SetHRange(trendTEC)
trendTEC.Draw()
c_TEC.SaveAs(folder+"/MergedBadChannelsTrends/MergedBadChannelsTEC_"+str(runmin).strip(' \n')+"_"+str(runmax).strip(' \n')+".png")
c_TEC.Write()
...
SetHRange(trendTID)
trendTID.Draw()
c_TID.SaveAs(folder+"/MergedBadChannelsTrends/MergedBadChannelsTID_"+str(runmin).strip(' \n')+"_"+str(runmax).strip(' \n')+".png")
c_TID.Write()
